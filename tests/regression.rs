...
/// Money typing must survive a used range that does not start at column A.
///
/// `xlmoney::money_columns` decodes sheet-*absolute* column indices from cell
/// references (`<c r="D10">` is column 3, whatever else is on the sheet), but
/// `RawTable`'s column 0 is wherever calamine's used range actually starts —
/// column A only when the sheet's data does too. `testdata/xl_money_offset_a.xlsx`
/// and `testdata/xl_money_offset_c.xlsx` (generated by
/// `testdata/gen/15_audit_defects.py`) are byte-for-byte the same four-column
/// table (`label`, `alpha`, `beta` both currency-formatted with names
/// `looks_monetary` would not catch and inconsistent scales so only the cell
/// format can make them decimal, `note`), differing only in whether that
/// table starts at sheet column A or column C. Before `RawTable::col_offset`
/// existed, the `_c` file's money columns were tested against the wrong table
/// index entirely — a silent miss here, and on an unluckier layout a silent
/// bind to the *wrong* column, which is the sharper failure this asserts
/// against by requiring the two files to type identically rather than just
/// checking one.
#[test]
fn money_typing_survives_a_used_range_that_does_not_start_at_column_a() {
    let a = Path::new("testdata/xl_money_offset_a.xlsx");
    let c = Path::new("testdata/xl_money_offset_c.xlsx");
    let spec_a = sniffed(a).spec;
    let spec_c = sniffed(c).spec;

    let dtypes = |spec: &ParseSpec| -> Vec<(String, DType)> {
        spec.columns.iter().map(|col| (col.name.clone(), col.dtype.clone())).collect()
    };
    assert_eq!(dtypes(&spec_a), dtypes(&spec_c), "the two files must type identically");

    for (spec, label) in [(&spec_a, "a"), (&spec_c, "c")] {
        for name in ["alpha", "beta"] {
            let col = spec.columns.iter().find(|c| c.name == name).unwrap();
            assert!(
                matches!(col.dtype, DType::Decimal { .. }),
                "xl_money_offset_{label}.xlsx: {name} typed {:?}, not decimal",
                col.dtype
            );
        }
    }
